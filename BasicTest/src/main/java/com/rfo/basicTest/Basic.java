...
public class Basic extends Activity {
	...
	public static final String SERVICE_DIR   = "Service_Programs";//2018-01-04gt
	public static final String SOURCE_SAMPLES_PATH = SOURCE_DIR + '/' + SAMPLES_DIR;
	public static final String SOURCE_SERVICE_PATH= SOURCE_DIR + '/' + SERVICE_DIR;//2018-01-04gt

	public static String AppPath = "rfo-basic";				// Set to the path name for application directories
	public static String AppName = "Basic!";				// 2017-12-22gt
	public static String AppInternalDataPath = "";				// Init an app internal data path //2017-03-15gt
	public static String AppPreferencePath = "";				// Init an Basic! own preference path //2018-01-04gt
	...
	public static String onStartAction = ""; //2017-02-22gt
	public static String onStartData = ""; //2017-02-22gt
	public static Bundle onStartExtras = null;  //2017-06-24gt
	public static Intent resultIntent = null;  //2017-07-10gt

	public static Activity mgActivity = null; //2017-06-24gt
	public static int mgPID = 0; //2017-06-24gt
	...
	public static String getFilePath(String subdir, String subPath) {
		String path = subPath;
		boolean goOn = true;// vv 2017-02-18gt
		if (path != null) {
	...
	public static String getServicePath(String subPath) {//vv2018-01-04gt
		return getFilePath(SOURCE_SERVICE_PATH, subPath);
	}//^^2018-01-04gt
	...
	private void initVars() {
		...
		AppName = res.getString(R.string.app_name); //2017-12-22gt
		...
		AppInternalDataPath = appContext.getFilesDir().toString(); //Create internal private directory //2017-03-15gt
		AppPreferencePath = appContext.getDir("preferences",MODE_PRIVATE).toString(); // Init an Basic! own preference path //2018-01-04gt
		DoAutoRun = false;
		
		mgActivity = this; //2017-06-24gt
		mgPID = android.os.Process.myPid(); //2017-06-24gt
	}
	...
	@Override
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {//vv2017-06-24gt
		switch (resultCode) {
		case 0:
			finish();
			break;
		case 88:
			...
			//resultIntent.putExtra("EXTRA_RESULT", "Exit at Basic class 88");
			setResult(RESULT_OK, resultIntent);
			finish();
			break;
		case 99:
			if (resultIntent != null) setResult(RESULT_OK, resultIntent);
			finish();
			break;
		}
	...
	private void createForSB() {							// Create code for Standard Basic
		...
		onStartAction = myIntent.getAction();//vv2017-02-22gt
		//onStartData = myIntent.getDataString();
		//if (onStartData == null)onStartData = ""; //2017-03-19gt
		if (myIntent.getDataString() != null) onStartData = myIntent.getDataString();
		try {
			onStartExtras = myIntent.getExtras();
			if (!onStartExtras.containsKey("\uffff")) onStartExtras.putBundle("\uffff", new Bundle());//2017-06-24gt
		} catch (Exception e) {
			onStartExtras = null;
		}//^^2017-02-22gt
		...
		if ((FileName != null) && !FileName.equals("")) {				// launched by shortcut or as a file share intent
				String mMessage = ""; //vv2017-06-19gt   Do not involve the Editor
				if(myIntent.getStringExtra("_BASIC!") != null)mMessage = myIntent.getStringExtra("_BASIC!");
			if (mMessage.toLowerCase().indexOf("_editor") == 0){
				Intent intent = new Intent(this, Editor.class);
				intent.putExtra("mProgPath", FileName);
				intent.putExtra("mEditor", true);
				intent.putExtra("mMessage", mMessage);//2018-01-04gt
				DoAutoRun = false;
				//startActivity(intent);										// run the program
				//finish();
				startActivityForResult(intent, 99); //17-06-24gt
			}else{//^^2017-06-19gt
		...
		} else if (AreSamplesLoaded() && AreServiceProgsLoaded()) {//2018-01-04gt								// this is not a launcher short cut
			DoAutoRun = false;
			Intent intent = new Intent(this, Editor.class);
	...
	private void createForAPK() {										// Create code for APK
		...
		Intent myIntent = getIntent();    // #219 Launched as a file share intent with an argument?//2017-02-21nm
		/* Deleted because BUNDLE.IN instead of command()
		String ArgPath = "";//vv2017-02-21nm
		if (myIntent.getData() != null) ArgPath = myIntent.getData().getPath();
		if (!ArgPath.equals("")) Run.called_with = getRelativePath(ArgPath, Basic.getDataPath(null));//^^2017-02-21nm
		*/
		
		onStartAction = myIntent.getAction();//vv2017-02-22gt
		//onStartData = myIntent.getDataString();
		//if (onStartData == null)onStartData = ""; //2017-03-19gt
		if (myIntent.getDataString() != null) onStartData = myIntent.getDataString();//2017-03-19gt
	...
	public static void clearProgram() {
		// Run.running_bas = "";//2017-02-21nm // deleted, because you can use file:// //2017-02-23gt
	...
	//vv2018-01-04gt
	private static boolean AreServiceProgsLoaded() { // Service program files have not been loaded
		// if the service programs directory is empty
		String servicePath = getServicePath(null);
		File sdDir = new File(servicePath);
		sdDir.mkdirs();
		return true;
	}
	//^^2018-01-04gt
	 ...
	 *
	 **/
	public static String getRelativePath(String targetPath, String basePath) {//vv2017-02-21mn
	...
	public static int loadProgramFileToList(boolean isFullPath, String path, ArrayList<String> list) {
		...
		// Save running bas path after making it relative to rfo-basic/source//vv2017-02-21nm
	...
	public static void loadProgramFromString(String text, AddProgramLine APL) {
		...
		for (int k = 0; k < length; ++k) {
			...
			if (c == '\n') {
				...
				sb.setLength(0);
			} else if (c == '\r'){ //Ignore Carriage Return CHR$(13)//2017-08-03gt
			} else {
				sb.append(c);
			}
		}
	...
	public class Loader extends AsyncTask<String, String, String>{
		...
		protected String doInBackground(String... str) {
			...
			startActivityForResult(doNext, 99); //17-06-24gt
		...
		private Intent doBGforSB() {								// Background code for Standard Basic
			...
			DoAutoRun = false;
			Settings.setLastLoadedProgramPath(getSourcePath(""), "");//2018-01-04gt
	...
	public static class ColoredTextAdapter extends ArrayAdapter<String> {
		...
		private double mTextHtml;//2018-02-01gt
		private double mTextSelectable;//2018-02-01gt
		private double mBmScale;//2018-02-01gt
		private String mTextDirection;//2018-02-01gt
		...
		public ColoredTextAdapter(Activity activity, ArrayList<String> alist, TextStyle style, Typeface typeface) {
			...
			mTextHtml = 0;//2018-02-01gt
			mTextSelectable = 0;//2018-02-01gt
			mBmScale = 0;//2018-02-01gt
			mTextDirection = "";//2018-02-01gt
		}
		...
		//vv2018-02-01gt
		public ColoredTextAdapter(Activity activity, ArrayList<String> alist, TextStyle style, 
				double textHtml, double textSelectable, double bmScale, String textDirection) {
			super(activity, Settings.getLOadapter(activity), alist);
			mActivity = activity;
			mList = alist;
			mTextStyle = style;
			...
			mBmScale = bmScale;//2018-02-01gt
			mTextDirection = textDirection;//2018-02-01gt
		}//^^2018-02-01gt

		public int getTextColor() { return mTextStyle.mTextColor; }
		public float getTextSize() { return mTextStyle.mSize; }//2017-10-13gt
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (mTextHtml > 0){		//vv2018-02-01gt
			/*
			 * https://developer.android.com/reference/android/text/Html.html
			 if (Build.VERSION.SDK_INT >= 24) {
				 text.setText(Html.fromHtml(mList.get(position)), 0); // for 24 api and more
			 } else {
				 */
				 //text.setText(Html.fromHtml(mList.get(position)));//2018-02-01gt // or for older api
			//	text.setText(Html.fromHtml(mList.get(position)));//2018-02-01gt // or for older api
				text.setText(
						Html.fromHtml(mList.get(position), new Html.ImageGetter() {
							@Override
							public Drawable getDrawable(String source) {
								...
								try {
									...
									// drawable = new BitmapDrawable(mContextMgr.getContext().getResources(), bm);
									drawable = new BitmapDrawable(bm);
									drawable.setBounds(0, 0, bm.getWidth(),
											bm.getHeight());
									*/
									/*
									// Smarter solution, but the returned bitmaps have a different size.
									// Unacceptable results on devices with height resolution.
									//Run.PrintShow(Basic.getFilePath(Basic.DATA_DIR, source));
									drawable = Drawable.createFromPath(Basic.getFilePath(Basic.DATA_DIR, source));
									drawable.setBounds(0, 0,
											drawable.getIntrinsicWidth(),
											drawable.getIntrinsicHeight());
									*/
									Bitmap bm = BitmapFactory.decodeFile(
											Basic.getFilePath(Basic.DATA_DIR, source));
									// convert Bitmap to Drawable
									// drawable = new BitmapDrawable(mContextMgr.getContext().getResources(), bm);
									drawable = new BitmapDrawable(bm);
									...
									if (mBmScale > 0){
										float ts = (float) (mTextStyle.mSize / bm.getHeight()* densityMultiplier * mBmScale) ;
										drawable.setBounds(0, 0, (int)(bm.getWidth()*ts),(int)(bm.getHeight()*ts));
									}else if(mBmScale == 0){
										drawable.setBounds(0, 0, (int)(bm.getWidth()*densityMultiplier),
												(int) (bm.getHeight() * densityMultiplier));
									}else{
										drawable.setBounds(0, 0, bm.getWidth(),bm.getHeight());
									}
								
								} catch (Exception e) {
									Run.PrintShow("Error, HTML Image Loading: <br>" + source + "<br>" + e);
								}
								return drawable;
							}
						}, null));
				...
				if (mTextSelectable > 0) {//2018-02-01gt
					if (Build.VERSION.SDK_INT >= 11)
						text.setTextIsSelectable(true); // API-11 and above
				} else {
					//vv On the first view the total opposite of the first thought
			...
			if (mTextDirection.equals("_RTL"))
				text.setTextDirection(View.TEXT_DIRECTION_RTL );//2018-02-01gt
			if (mTextDirection.equals("_IfAnyRTL"))
				text.setTextDirection(View.TEXT_DIRECTION_ANY_RTL );//2018-02-01gt
			text.setTextSize(mTextStyle.mSize);//2017-10-13gt
