...
//2017-03-12gt	//private final Bundle mDataBundle;	//or String?						// RUN command can add a data bundle//2017-02-25gt

	public AutoRun(Context context, String filename, boolean fromRun, String data) {
		//public AutoRun(Context context, String filename, boolean fromRun, String data, Bundle dataBundle) { //2017-02-25gt
		Log.v(AutoRun.LOGTAG, filename);
		mContext = context;
		mFileName = filename;
		mFromRun = fromRun;
		mData = data;
//2017-03-12gt		// mDataBundle = dataBundle; //2017-02-25gt
	}
...
// TODO:	The loading should be in a background thread.
			...
			/*
			if (mDataBundle != null) {						// if RUN command added a data bundle //vv2017-02-25gt
				String data = "##@=\"" + mDataBundle + "\"";
				AddProgramLine.charCount = data.length();
				APL.AddLine(data);
			} //^^2017-02-25gt
			 */
	...
	private boolean FileLoader(String aFileName) {					// Loads the selected file
		...
		if (!mFromRun) {											// file name from shortcut may need massage
			...
			if (aFileName.toLowerCase().indexOf("file://") == 0)aFileName = aFileName.substring(7);//2017-02-23gt
		}

		Basic.clearProgram(); 				// Clear the old program
		Editor.DisplayText = "";			// Clear the display text buffer
		//Run.PrintShow("");//2017-02-23 Without, BASIC! crashes.
		//Maybe deleting two wrong lines (mDataBundle) on top solves the issue, too.//2017-03-12
